...
contract PaymentHandler is Ownable, ReentrancyGuard {
     ...
     */
    function payForService(string memory _serviceType) external payable nonReentrant returns (bytes32) {
        uint256 requiredFee = getServiceFee(_serviceType);
        require(msg.value >= requiredFee, "Insufficient payment");
        
        bytes32 paymentId = keccak256(abi.encodePacked(msg.sender, _serviceType, block.timestamp, block.prevrandao));
    ...
    /**
     * @dev Complete a service (callable by authorized services)
     */
    function completeService(bytes32 _paymentId) external onlyOwner {
        ...
        emit ServiceCompleted(_paymentId, payments[_paymentId].user, payments[_paymentId].serviceType);
    }
     ...
     */
    function getServiceFee(string memory _serviceType) public view returns (uint256) {
        if (keccak256(abi.encodePacked(_serviceType)) == keccak256(abi.encodePacked("ai_generation"))) {
            return aiGenerationFee;
        } else if (keccak256(abi.encodePacked(_serviceType)) == keccak256(abi.encodePacked("storage"))) {
            return storageFee;
        } else if (keccak256(abi.encodePacked(_serviceType)) == keccak256(abi.encodePacked("mint"))) {
            return mintFee;
        } else if (keccak256(abi.encodePacked(_serviceType)) == keccak256(abi.encodePacked("remix"))) {
            return remixFee;
        }
